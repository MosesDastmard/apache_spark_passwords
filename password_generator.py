...
if __name__ == '__main__':
    ...
    if mode != 'spark':
        ...
        spark = SparkSession.builder.master("local[16]").appName('Spark Password Generator').getOrCreate()
        sparkContext = spark.sparkContext
        sparkContext.parallelize([n]*N).map(gen_pass).coalesce(16).saveAsTextFile(output_path)
